...
// Map Interface Code; disabled for now
...
function filterData(filters) {
    ...
    if (filters) {
        for (var key in _collab_data['entity']) {
            var row = _collab_data['entity'][key];
            // Check filters
            if (row['entity_type'] == 'organization'){
                var addToDisplay = false;
            } else {
...
function refreshTableData(query) {
    ...
    if (_selected_collab_list.length <= 5) {
        for (collab_key in _selected_collab_list) {
            for (o_key in _collab_data['entity-membership'][_selected_collab_list[collab_key]]) {
                var org_key = _collab_data['entity-membership'][_selected_collab_list[collab_key]][o_key];
                var name = _collab_data['entity'][org_key]['entity_name'];
                organization_list += name + '|' + org_key + '||';
            }
        }
...
function createFilterFor(key) {
    ...
    if (key === 'primary_focus') {
        ...
        for (var i in focuses) {
            options.push(focuses[i]);
        }    
    } else {
        for (var collab in _collab_data['entity']) {
            var value = '';
            if (_collab_data['entity'][collab]['entity_type'] === 'collaboration') {
                value = _collab_data['entity'][collab][key];
            }
...
function getOrganizationData(key, callback) {
    ...
    $.ajax({
        type: "GET",
        dataType: "jsonp",
        url: "https://atlas-769c1.firebaseio.com/entity/" + key + "/.json?",
        success: function(data) {
            if (data) {
                data['key'] = thisKey;
                callback(data);
...
function createMapInfoContent(data) {
    ...
    if (data && data.record) {
        ...
        content = '<div id="content">' + '<div><h4>' + data.record.entity_name + '</h4><p>' + about + '</p><p><a href="http://www.sciencepolicyatlas.com/collaboration?collab=' + data.record.key + '/" target="_blank">Learn more</a>' + '</p>';
    }
    return content;
}
...
function addMapMarker(data) {
    if (data) {
        var title = data.record.entity_name;
        var infoContent = createMapInfoContent(data);
        var marker = new google.maps.Marker({
            position: data.latlng,
            map: map,
            title: title,
            label: data.record.entity_category[0],
            animation: google.maps.Animation.DROP
        });
        _markers.push(marker);
        bindContentToInfoWindow(marker, map, infowindow, infoContent);
