...
function createFilterFor(key) {
    ...
    var options = [];
    for (var collab in _data) {

        // multiple collaborations on one line comma delimited
        var values = _data[collab][key] ? _data[collab][key].split(", ") : "";
...
$(window).load(function() {
    ...
    var config = {
    apiKey: "AIzaSyBy82RnwEn64kgY-Ojs-VFTNtreCJhEPlw",
    authDomain: "atlas-new-format.firebaseapp.com",
    databaseURL: "https://atlas-new-format.firebaseio.com",
    storageBucket: "atlas-new-format.appspot.com",
    messagingSenderId: "820752435938"
    };
    ...
    //var test_var = require('https://atlas-new-format.firebaseio.com/organizations/.json');
    // Download the Firebase data for this specific Key
    getCollaborationData(key, hydrateView);
});

function hydrateView(data, alist) {
    if (data) {
        displayMap();
        getGeolocationAddMapMarker(data);
        //getAddCollaborationMapMarkers(data.collaboration_links);
        //getAddOrganizationMapMarkers(data.organization_links);
    }
}
